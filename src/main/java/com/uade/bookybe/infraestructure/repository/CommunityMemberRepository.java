...
@Repository
public interface CommunityMemberRepository extends JpaRepository<CommunityMemberEntity, CommunityMemberId> {

  List<CommunityMemberEntity> findByCommunityId(String communityId);
  
  List<CommunityMemberEntity> findByUserId(String userId);
  
  @Query("SELECT cm FROM CommunityMemberEntity cm JOIN FETCH cm.user WHERE cm.communityId = :communityId")
  List<CommunityMemberEntity> findByCommunityIdWithUser(@Param("communityId") String communityId);
  
  @Query("SELECT cm FROM CommunityMemberEntity cm JOIN FETCH cm.community WHERE cm.userId = :userId")
